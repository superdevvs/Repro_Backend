...
| Dropbox | Historical 6 September observation: app ID/secret configured; no environment access/refresh token; zero OAuth rows. Active configuration was cleared and the dashboard integration retired on 7 September; see below. | Historical app/grant ownership and revocation remain unverified | Verify every external consumer and historical grant with the provider owner; revoke retired grants and retire or rotate an exposed app secret as appropriate. Dashboard removal is not provider revocation. |
...
## Dropbox retirement update, 2026-09-07

The dashboard integration is retired in live backend `80672ae7cc5f0a846bcbab2c5a4c7ac8af184f6f` and frontend `c6f8f1c16d3e6062a533ec0e6ca617a79e4f0370`. Before removal, read-only production inspection found zero Dropbox OAuth rows, no environment access/refresh tokens and no Dropbox paths or IDs among 231 shoot files. On 7 September at 03:52:32 UTC, the guarded operation removed active `DROPBOX_CLIENT_ID` and `DROPBOX_CLIENT_SECRET`, set `DROPBOX_ENABLED=false`, rebuilt configuration and signaled queue restart. The subsequent code release removed connection/import surfaces and provider configuration. No provider call, account deletion, cloud-file deletion or credential rotation was performed.

Historical app secrets and grants remain **unverified at the provider**. Local absence does not prove that a copied archive credential, an external consumer or a historical refresh grant has stopped working. The provider owner must establish those consumers and record revocation or app-retirement evidence before closing this part of #3. Reconnecting the retired dashboard is not part of that procedure. Application retirement and media/download acceptance are complete under #5, as recorded in `dropbox-retirement.md`; this does not change credential revocation status.
...
Inventory and strictly migrate Google Calendar access/refresh tokens, any historical shared Dropbox `encrypted:v1:` values still present, settings `enc:` fields, MFA secrets and encrypted recovery arrays, and private tax-document notes (`user_tax_documents.notes`). The pre-retirement production inventory found zero Dropbox OAuth rows; retain the format check for older restores or other inventoried consumers rather than assuming historical ciphertext cannot exist. Invalidate expiring encrypted MFA enrollment, any residual historical Dropbox OAuth cache entries and generic upload-source OAuth state. Use explicit old/new encrypters, checkpointed row locks or a maintenance window, and a protected database backup with a rehearsed restore. Abort on any decryption failure: never use the settings helper that substitutes empty strings. Verify migrated values using the new key alone before cutover.
...
| Historical studio Dropbox `oauth_tokens.access_token`, `.refresh_token`, if present in the maintenance/restore inventory | Zero production rows observed before retirement; for any historical rows, preserve `encrypted:v1:` prefix and distinguish legacy plaintext explicitly |
...
| Pending MFA setup, residual historical Dropbox OAuth, upload-source OAuth, encrypted cookies and signed links | Invalidate; do not migrate old browser authorization state into a new trust window |
