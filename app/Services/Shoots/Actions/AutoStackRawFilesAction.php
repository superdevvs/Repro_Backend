...
class AutoStackRawFilesAction
{
    ...
    public function execute(Shoot $shoot, bool $updateShootBracketMode = true, ?int $shootServiceId = null): array
    {
        return DB::transaction(function () use ($shoot, $updateShootBracketMode, $shootServiceId) {
            ...
            // A stack number is only meaningful for bracketed photo capture. Two kinds
            // of raw file must never acquire one:
            //
            //   - anything belonging to a service the catalogue does not bracket, such
            //     as drone or flash photography, which delivers one raw per final photo
            //   - video raws, including those on a bundled service that legitimately
            //     serves both lanes and therefore shares one execution row with its
            //     photo work
            //
            // Without this, EXIF clustering numbered them anyway, which made
            // non-bracket work look like one-frame stacks.
            $files = $this->rejectUnstackableFiles($shoot, $files);
    ...
    /**
     * Drop raw files that cannot belong to a bracket stack.
     *
     * Assigned files are judged by their service's capability; unassigned files (no
     * execution row) keep the legacy behaviour of being clustered against each other,
     * because there is no capability to consult for them.
     *
     * @param  Collection<int, ShootFile>  $files
     * @return Collection<int, ShootFile>
     */
    private function rejectUnstackableFiles(Shoot $shoot, Collection $files): Collection
    {
        ...
        $bracketsByItemId = $serviceIds->isEmpty()
            ? collect()
            : $shoot->serviceItems()
                ->with('service')
                ->whereIn('id', $serviceIds->all())
                ->get()
                ->mapWithKeys(fn ($item) => [(int) $item->id => $resolver->serviceUsesBrackets($item)]);

        return $files->reject(function (ShootFile $file) use ($bracketsByItemId) {
            if ($this->isVideoFile($file)) {
                return true;
            }

            $itemId = (int) ($file->shoot_service_id ?? 0);
            if ($itemId === 0) {
                return false;
            }

            return ! ($bracketsByItemId[$itemId] ?? false);
        })->values();
    }

    private function isVideoFile(ShootFile $file): bool
    {
        foreach ([$file->mime_type, $file->file_type] as $candidate) {
            if (is_string($candidate) && str_starts_with(strtolower($candidate), 'video/')) {
                return true;
            }
        }
