...
/**
 * Download CubiCasa deliverables (merged floor plan PDFs, per-floor JPGs,
 * home report PDFs, etc.) and persist them as ShootFile rows of
 * media_type=floorplan so they appear in Media -> Floorplans, Download
 * Center and the Dropbox mirror.
 *
 * Mirrors {@see IngestIguideAssetsJob}; consumes the floorplan list shape
 * produced by {@see \App\Services\CubiCasaService::buildFloorplanList()}.
 */
class IngestCubiCasaAssetsJob implements ShouldQueue
{
    ...
    public function handle(ShootActivityLogger $activityLogger): void
    {
        ...
        // Only ingest for shoots that booked a CubiCasa-eligible service.
        if (!$shoot->hasCubiCasaEligibleService()) {
            Log::info('IngestCubiCasaAssetsJob: skipped, no eligible service', [
                'shoot_id' => $shoot->id,
            ]);
            return;
        }
        ...
        $existingByKey = ShootFile::query()
            ...
            ->keyBy(function (ShootFile $f) {
                ...
                return $metadata['cubicasa_asset_key'] ?? null;
            })
        ...
        foreach ($this->floorplans as $item) {
            ...
            try {
                ...
                $filename = $this->sanitizeFilename(
                    $item['filename'] ?? basename(parse_url($url, PHP_URL_PATH) ?: $url) ?: 'cubicasa-asset'
                );
                ...
                $storedFilename = sprintf(
                    '%s-%s.%s',
                    Str::slug(pathinfo($filename, PATHINFO_FILENAME) ?: 'cubicasa'),
                ...
                if (!$response->successful()) {
                    Log::warning('IngestCubiCasaAssetsJob: download failed', [
                        'shoot_id' => $shoot->id,
                        'asset_key' => $assetKey,
        ...
        if (!empty($ingestedFileIds)) {
            try {
                ...
                Log::warning('IngestCubiCasaAssetsJob: activity log failed', [
                    'shoot_id' => $shoot->id,
                    'error' => $e->getMessage(),
                ]);
            }
    ...
    {
        Log::error('IngestCubiCasaAssetsJob: failed permanently', [
