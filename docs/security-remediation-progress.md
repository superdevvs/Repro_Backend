...
- #9 authentication: eight-character new-password policy, shared atomic rate limits, atomic reset consumption, optional MFA and phased verification are prepared in isolated checkouts based on the current #10 revisions. Focused validation passed 26 backend tests / 318 assertions and 14 frontend tests; TypeScript and lint passed. Two pre-existing resend fixtures now explicitly model unverified ownership, and all 24 email-health tests / 207 assertions pass. Full release validation is in progress. The pilot remains inactive pending deployed delivery/correction checks; a controlled test inbox has been authorized.
