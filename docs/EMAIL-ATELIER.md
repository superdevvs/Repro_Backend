...
- `EmailPresentation` applies the approved palette, spacing, semantic detail rows, and buttons to existing dynamic content. The desktop card is 640px within a 720px canvas; mobile uses 16px outer padding. Explicit preview themes override browser appearance; sent emails use light inline styles with adaptive dark rules.
...
Saved and unsaved previews use the same server renderer as sending. The editor has light/dark and 720/375px controls. The body region is marked `data-email-content="true"`, allowing an older full HTML document to reopen as editable content without duplicating the masthead or footer.
...
The release render inventory checks all 72 variants across 720/375px and light/dark, with original image bytes and external network blocked. Browser checks include content-region count, image loading, and element bounds. Chromium verification does not replace testing in every receiving email application.
