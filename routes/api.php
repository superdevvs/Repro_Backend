...
Route::middleware(['auth:sanctum', 'permission:voice-calls'])->prefix('voice')->group(function () {
    ...
    Route::get('calls/insights', [VoiceCallController::class, 'insights']);
    ...
    Route::post('calls/{call}/cockpit-opened', [VoiceCallController::class, 'cockpitOpened'])->middleware('permission:voice-calls,operate');
    Route::patch('calls/{call}/wrap-up', [VoiceCallController::class, 'wrapUp'])->middleware('permission:voice-calls,operate');
    Route::post('calls/{call}/note', [VoiceCallController::class, 'addNote'])->middleware('permission:voice-calls,operate');
    Route::post('calls/outbound', [VoiceCallController::class, 'outbound'])->middleware('permission:voice-calls,operate');
    Route::post('calls/{call}/hangup', [VoiceCallController::class, 'hangup'])->middleware('permission:voice-calls,operate');
    Route::post('calls/{call}/page-staff', [VoiceCallController::class, 'pageStaff'])->middleware('permission:voice-calls,operate');
    Route::post('calls/{call}/transfer', \App\Http\Controllers\API\Voice\VoiceTransferController::class)->middleware('permission:voice-calls,operate');
    ...
    Route::post('schedule/overrides', [VoiceScheduleController::class, 'store'])->middleware('permission:voice-calls,manage');
    Route::delete('schedule/overrides/{override}', [VoiceScheduleController::class, 'destroy'])->middleware('permission:voice-calls,manage');
    Route::get('llm-usage', VoiceLlmUsageController::class);
    Route::get('scheduled-calls', [ScheduledVoiceCallController::class, 'index']);
    Route::get('automation-rules', [\App\Http\Controllers\API\Voice\VoiceAutomationController::class, 'index']);
    Route::get('automation-rules/runs', [\App\Http\Controllers\API\Voice\VoiceAutomationController::class, 'runs']);
    Route::post('automation-rules/preview', [\App\Http\Controllers\API\Voice\VoiceAutomationController::class, 'preview'])->middleware('permission:voice-calls,manage');
    Route::post('automation-rules', [\App\Http\Controllers\API\Voice\VoiceAutomationController::class, 'store'])->middleware('permission:voice-calls,manage');
    Route::patch('automation-rules/tasks/{task}', [\App\Http\Controllers\API\Voice\VoiceAutomationController::class, 'updateTask'])->middleware('permission:voice-calls,operate');
    Route::patch('automation-rules/{rule}', [\App\Http\Controllers\API\Voice\VoiceAutomationController::class, 'update'])->middleware('permission:voice-calls,manage');
    Route::delete('automation-rules/{rule}', [\App\Http\Controllers\API\Voice\VoiceAutomationController::class, 'destroy'])->middleware('permission:voice-calls,manage');
    Route::post('scheduled-calls', [ScheduledVoiceCallController::class, 'store'])->middleware('permission:voice-calls,operate');
    Route::patch('scheduled-calls/{scheduledCall}', [ScheduledVoiceCallController::class, 'update'])->middleware('permission:voice-calls,operate');
    Route::post('scheduled-calls/{scheduledCall}/cancel', [ScheduledVoiceCallController::class, 'cancel'])->middleware('permission:voice-calls,operate');
    Route::post('scheduled-calls/{scheduledCall}/retry', [ScheduledVoiceCallController::class, 'retry'])->middleware('permission:voice-calls,operate');
    Route::get('numbers', [VoiceNumberController::class, 'index']);
    Route::patch('numbers/{smsNumber}', [VoiceNumberController::class, 'update'])->middleware('permission:voice-calls,manage');
    Route::get('handoffs/recent', [VoiceHandoffController::class, 'index']);
    Route::get('settings', [VoiceSettingsController::class, 'show']);
    Route::patch('settings', [VoiceSettingsController::class, 'update'])->middleware('permission:voice-calls,manage');
    Route::post('assistant/sync', VoiceAssistantSyncController::class)->middleware('permission:voice-calls,manage');
});
